...
export function ReservationLocationDetails({
  location,
  expanded,
  onToggle,
}: ReservationLocationDetailsProps) {
  const showCoordinates = hasValidCoordinates(
    location.latitude,
    location.longitude,
  );
    ...
    <View style={styles.container}>
      ...
      {expanded ? (
        <View style={styles.details}>
          ...
          {location.address ? (
            <Detail label="Dirección" value={location.address} />
          ) : null}
          {showCoordinates ? (
            <Detail
              label="Coordenadas"
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <View style={styles.row}>
      ...
      <AppText variant="body" color={colors.primary} style={styles.strong}>
        {value}
      </AppText>
    </View>
  );
