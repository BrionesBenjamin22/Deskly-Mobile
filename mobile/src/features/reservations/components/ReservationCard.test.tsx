...
import { buildReservation, buildReservationWithPartialLocation, buildReservationWithoutLocation } from '../testing/reservation.fixtures';
...
describe('ReservationCard location details', () => {
  it('abre y cierra los datos geograficos sin ocultar el resumen', () => {
    ...
    const toggle = screen.getByRole('button', { name: 'Ver detalles de ubicación' });
  ...
  it('omite valores opcionales ausentes y no ofrece detalle sin ubicacion', () => {
    ...
    expect(screen.queryByRole('button', { name: 'Ver detalles de ubicación' })).not.toBeOnTheScreen();
  });

  it('mantiene independiente el detalle de cada tarjeta', () => {
    render(<ReservationList reservations={[
      buildReservation(),
      buildReservation({ id: 'reservation-2', deskName: 'Escritorio patio', location: { areaId: 'area-2', areaName: 'Sala Sur', localityId: 'locality-2', localityName: 'La Plata' } }),
    ]} />);

    fireEvent.press(screen.getAllByRole('button', { name: 'Ver detalles de ubicación' })[1]);
